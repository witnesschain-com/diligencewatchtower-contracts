...
const deployFn = async (hre) => {
    ...
    const Deployment__OptimistInviterImpl = await hre.deployments.get('OptimistInviter');
    ...
    const { deploy } = await hre.deployments.deterministic('OptimistInviterProxy', {
        salt: hre.ethers.utils.solidityKeccak256(['string'], ['OptimistInviterProxy']),
        contract: 'Proxy',
        from: deployer,
        args: [deployer],
        log: true,
    ...
    await deploy();
    const Deployment__OptimistInviterProxy = await hre.deployments.get('OptimistInviterProxy');
    console.log(`OptimistInviterProxy deployed to ${Deployment__OptimistInviterProxy.address}`);
    // Deployed Proxy.sol contract
    const Proxy = await hre.ethers.getContractAt('Proxy', Deployment__OptimistInviterProxy.address);
    // Deployed Proxy.sol contract with the OptimistInviter interface
    const OptimistInviter = await hre.ethers.getContractAt('OptimistInviter', Deployment__OptimistInviterProxy.address);
    const name = deployConfig.optimistInviterName;
    // Create the calldata for the call to `initialize()`
    const initializeCalldata = OptimistInviter.interface.encodeFunctionData('initialize', [name]);
    ...
    await (0, deploy_utils_1.assertContractVariable)(OptimistInviter, 'version', '1.0.0');
    await (0, deploy_utils_1.assertContractVariable)(OptimistInviter, 'INVITE_GRANTER', deployConfig.optimistInviterInviteGranter);
    await (0, deploy_utils_1.assertContractVariable)(OptimistInviter, 'ATTESTATION_STATION', Deployment__AttestationStation.address);
    await (0, deploy_utils_1.assertContractVariable)(OptimistInviter, 'EIP712_VERSION', '1.0.0');
};
